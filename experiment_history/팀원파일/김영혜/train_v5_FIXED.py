...
# ============================================================================
# [FIXED] 범주형 컬럼 변환 및 카테고리 일치
# ----------------------------------------------------------------------------
# 원본 코드는 train+test 합집합으로 카테고리를 만들어서 DACON 규칙
# ("label/one-hot encoding 시 test 데이터셋 활용" = Data Leakage) 위반이었음.
# train에만 fit, test의 새 값은 train 최빈값으로 대체 (test 통계 미사용).
# ============================================================================
cat_cols = []
for col in features:
    if not pd.api.types.is_numeric_dtype(X[col]) and not pd.api.types.is_bool_dtype(X[col]):
        ...
        # ★ train에만 fit (test 데이터를 카테고리 구조 결정에 전혀 사용하지 않음)
        train_categories = sorted(X[col].unique().tolist())
        fallback = X[col].value_counts().idxmax()
        # test에만 있는(train에 없는) 새 값은 train 최빈값으로 대체
        X_test[col] = X_test[col].where(X_test[col].isin(train_categories), fallback)
...
# 9. 최종 제출 파일 저장 (probability 컬럼 지정 및 확률값 그대로 제출)
