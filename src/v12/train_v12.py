...
def preprocess_data(df, transfer_median):
    ...
    # 3.10 [신규] 1등 솔루션의 실질 가임 연령 및 리쥬브네이션 갭 피처 추가
    patient_mid = {
    ...
    # [제거] 비율 피처 제거 및 결측 플래그 제거, infertility_severity_score 제거
    # 원래 나이 컬럼 및 횟수 컬럼 드롭
...
for col in features:
    if not pd.api.types.is_numeric_dtype(X[col]) and not pd.api.types.is_bool_dtype(X[col]):
        cat_cols.append(col)
        # 결측치를 NaN으로 유지하면서 카테고리 정의
        X[col] = X[col].astype(str)
        X_test[col] = X_test[col].astype(str)
        # NaN 문자열이 있으면 원래대로 진짜 NaN(np.nan)으로 복원
        ...
        # Categorical 타입으로 변환 (Train과 Test가 동일한 categories 풀을 공유하게 함)
...
class ECDFReference:
    ...
    def transform(self, x):
        ...
        ranks = (left + right) / 2.0  # 평균 순위 (동점 처리)
        return ranks / self.n          # [0,1] 범위 랭크 변환

# ==================== 1차 모델 학습 준비 ====================
folds = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# 모델별 OOF 및 Test 예측값을 담을 딕셔너리
model_oofs = {}
model_tests = {}
...
# MLP용 인코딩 함수 (unseen -1, missing -2 보존)
...
# ==================== 5. Leak-Free ECDF Rank Blending ====================
print("\n--- Applying Leak-Free ECDF Rank Blending ---")

model_names = ['lgbm', 'xgboost', 'catboost', 'mlp']
weights = {'lgbm': 0.40, 'xgboost': 0.20, 'catboost': 0.20, 'mlp': 0.20}
...
for fold, (train_idx, val_idx) in enumerate(folds.split(X, y)):
    for name in model_names:
        # 각 fold-train 예측값 분포로 ECDFReference fit
        ...
        # Test 해당 fold 예측값 랭크 변환 후 분할 기여
...
for name in model_names:
    final_oof_rank += weights[name] * rank_oofs[name]
    final_test_rank += weights[name] * rank_tests[name]

final_score = roc_auc_score(y, final_oof_rank)
print(f"\nFinal ECDF Blended Rank OOF AUC: {final_score:.6f}")

# ==================== 6. Save Submission ====================
submission['probability'] = final_test_rank
output_sub_dir = os.path.join(BASE_DIR, "src", "v12")
os.makedirs(output_sub_dir, exist_ok=True)

output_sub_path = os.path.join(output_sub_dir, f"submission_v12_ens_{final_score:.6f}.csv")
submission.to_csv(output_sub_path, index=False)
print(f"\nSaved v12 ECDF Blended Submission to: {output_sub_path}")

# 백업용으로 데이터 폴더에도 저장
backup_sub_path = os.path.join(DATA_DIR, f"submission_v12_ens_{final_score:.6f}.csv")
