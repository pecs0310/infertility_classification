...
lgb_params = {
    ...
    'metric': 'auc',
    'boosting_type': 'gbdt',
    'learning_rate': 0.02,        # 학습률 낮춤 (0.05 -> 0.02)
    'num_leaves': 63,             # 용량 증대 (31 -> 63)
    'max_depth': 9,               # 트리 깊이 명시적 제어
    'scale_pos_weight': 1.5,      # 불균형 조절 (1.0 -> 1.5)
    'feature_fraction': 0.7,      # 무작위 정규화 (0.8 -> 0.7)
...
for fold, (train_idx, val_idx) in enumerate(folds.split(X, y)):
    ...
    callbacks = [
        ...
        lgb.log_evaluation(period=0) # 출력 비활성화
    ]
...
# XGBoost는 범주형 변수를 카테고리 데이터타입을 활용해 바로 학습할 수 있습니다.
for fold, (train_idx, val_idx) in enumerate(folds.split(X, y)):
    X_tr, y_tr = X.iloc[train_idx], y.iloc[train_idx]
    X_va, y_va = X.iloc[val_idx], y.iloc[val_idx]
    ...
    model = xgb.XGBClassifier(
        ...
        tree_method='hist',
        enable_categorical=True, # 카테고리 데이터 피처 처리 허용
        random_state=42,
        n_jobs=-1,
        eval_metric='auc',
...
# CatBoost는 category 타입을 문자열로 인식하게 한 뒤 cat_features로 직접 넘겨야 에러가 안 납니다.
# 학습 시에만 복사본을 생성해 cat_features 인덱스를 전달합니다.
X_cat = X.copy()
...
# 각 모델의 성능 편차에 맞추어 최적 가중치 부여 (LightGBM: 40%, XGBoost: 30%, CatBoost: 30%)
...
output_sub_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "submission_v3_ensemble.csv")
