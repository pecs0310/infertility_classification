...
def build_features(df, transfer_median):
    ...
    age_map = {
        ...
        '만40-42세': 3, '万43-44세': 4, '만45-50세': 5, '알 수 없음': np.nan
    }
    donor_age_map = {
...
# v18 모델 정의 (lgbm_spw2 제외, rf/et 모델 추가)
...
if os.path.exists(oof_cache_path):
    ...
    # 캐시 파일 포맷 마이그레이션 (rf, et 신규 모델 추가에 따른 누락 키 방지)
    for s in seeds:
...
for seed in seeds:
    # 이미 완료된 시드 체크포인트 스킵 (모든 모델 결과가 찬 경우만)
    all_done = True
    for name in model_names:
    ...
    # 7. MLPClassifier (v18 원핫인코딩 반영)
    if np.all(model_oofs_seeds[seed]['mlp'] == 0):
        print(f"\n--- [Seed {seed}] Model 7: MLPClassifier (One-Hot Encoded) ---")
        for fold, (train_idx, val_idx) in enumerate(folds.split(X, y)):
            X_tr, y_tr = X.iloc[train_idx], y[train_idx]
            X_va, y_va = X.iloc[val_idx], y[val_idx]
...
output_sub_path = f"submission_v18_bag_{final_auc:.6f}.csv"
submission.to_csv(output_sub_path, index=False)
print(f"\nSaved v18 ECDF Bagged Ridge Stack Submission to: {output_sub_path}")
