...
def preprocess_data(df):
    ...
    # 3.2 고령 임산부 기준 플래그 생성
    df_new['is_advanced_age'] = (df_new['시술 당시 나이_ordinal'] >= 1).astype(int)  # 만 35세 이상
    df_new['is_elderly_age'] = (df_new['시술 당시 나이_ordinal'] >= 3).astype(int)   # 만 40세 이상 (초고령)
    ...
    # 3.4 [v4] 파생 피처: 이전 성공률 (출산 횟수 / (시술 횟수 + 1))
    ...
    # 3.5 [v4] 파생 피처: 고령 임산부 및 기증 난자 상호작용 피처
    # 고령 여성이 기증난자를 썼을 때와 본인난자를 썼을 때의 성공 예후 차이를 적극 반영
    ...
    # 젊은 기증자(만 30세 이하) 난자를 기증받아 사용했는지 여부
    df_new['young_donor_egg'] = ((df_new['난자 출처'] == '기증 제공') & (df_new['난자 기증자 나이_ordinal'] <= 2)).astype(int)
    
    # 3.6 [v4] 파생 피처: 이식 미실시 및 동결 전용 주기 플래그
    # 이식 경과일 결측(-1)이거나 이식 배아수가 0개이면 실질적으로 이식이 이루어지지 않아 임신 가능성이 극도로 낮아짐
    ...
    # 3.7 파생 피처: 배아 배양 일수 계산 (배아 이식 경과일 - 난자 혼합 경과일)
    ...
    # 3.8 [v4] 파생 피처: 난수/배아 관련 비율 지표 세분화
    ...
    # 3.9 시술 특성별 결측치 조건부 처리
    # 3.9.1 '동결 배아 사용 여부' == 1인 경우, 신선 난자 관련 피처의 결측을 -1로 대치
    ...
    # 3.9.2 '신선 배아 사용 여부' == 1인 경우, 해동 관련 피처의 결측을 -1로 대치
    ...
    # 3.11 기존 원본 컬럼 중 수치화된 컬럼들은 피처 다중 공선성 예방을 위해 제거
...
# 4. 범주형 컬럼 변환 및 결측치 제거 (XGBoost/CatBoost의 카테고리 불일치 예외 방지)
cat_cols = []
for col in features:
...
print(f"Total features after v4 domain engineering: {len(features)}")
print(f"Categorical features: {len(cat_cols)}")

# K-Fold 기본 설정
folds = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# ==================== 5. Model 1: LightGBM (Tuned) ====================
...
# ==================== 6. Model 2: XGBoost (Categorical Native) ====================
...
print(f"LightGBM 가중치: {w_lgb:.2f} | XGBoost 가중치: {w_xgb:.2f} | CatBoost 가중치: {w_cat:.2f}")
...
output_sub_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "submission_v4_ensemble.csv")
