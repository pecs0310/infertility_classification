...
OUTPUT_DIR = os.path.join(BASE_DIR, "src", "v16")

# File names
V16_FILE = os.path.join(DATA_DIR, "submission_v16_bag_0.740587.csv")
TEAM_BEST_FILE = os.path.join(DATA_DIR, "team_best_submit_0.741849.csv")

print(f"Loading v16 submission from: {V16_FILE}")
print(f"Loading Team Best submission from: {TEAM_BEST_FILE}")

# Load submissions
df_v16 = pd.read_csv(V16_FILE)
df_team = pd.read_csv(TEAM_BEST_FILE)

# Validate alignment
assert (df_v16["ID"] == df_team["ID"]).all(), "IDs are not aligned between submissions!"
print(f"Loaded successfully. Total rows: {len(df_v16)}")
...
ecdf_v16 = ECDFReference(df_v16["probability"])
ecdf_team = ECDFReference(df_team["probability"])

rank_v16 = ecdf_v16.transform(df_v16["probability"])
...
# Define blend weights (w_v16, w_team)
blend_weights = [
    (0.5, 0.5, "50_50"),
    (0.4, 0.6, "40_60"),
    (0.3, 0.7, "30_70"),
...
for w_v16, w_team, suffix in blend_weights:
    print(f"\nBlending with weights: v16 ({w_v16:.1f}) + Team Best ({w_team:.1f})")
    
    # Blended rank
    blended_rank = w_v16 * rank_v16 + w_team * rank_team
    ...
    # Save files
    out_src = os.path.join(OUTPUT_DIR, f"submission_v16_blend_{suffix}.csv")
    out_data = os.path.join(DATA_DIR, f"submission_v16_blend_{suffix}.csv")
